...
- `multer` nếu cần upload asset sau này (chưa bắt buộc MVP).

Backend packages cần cài: `express cors dotenv axios`.

Port mặc định: backend `3000`, frontend `5173`.

Vite proxy bắt buộc — coder phải cấu hình `vite.config.js` để `/api/*` forward về `http://localhost:3000`, tránh lỗi CORS khi dev:
...
Nếu backup thất bại (disk đầy, lỗi permission): **không tiếp tục ghi lesson mới**, trả lỗi `500` với message rõ ràng. Thà mất một lần authoring còn hơn ghi đè dữ liệu mà không có backup.
...
|---|---|
| Thành công | 200 |
| Request thiếu field | 400 |
| Validate lesson fail | 422 |
| LLM lỗi hoặc timeout | 502 |
| Lỗi server / backup fail | 500 |
...
- Mọi ghi lesson phải validate trước, backup trước, ghi sau. Backup fail thì không ghi.
...
- Audio cũ phải dừng khi chuyển slide hoặc khi video bắt đầu phát.
- `BUILD_LOG.md` phải được tạo từ TASK-001 và cập nhật sau mỗi task hoàn thành.
